...
docker rm -f marqo-os &&
    docker run --platform linux/amd64 --name marqo-os -id -p 9200:9200 -p 9600:9600 -e "discovery.type=single-node" marqoai/marqo-os:0.0.2 &
...
docker rm -f marqo
    docker run --name marqo --privileged -p 8882:8882 --add-host host.docker.internal:host-gateway \
        -e "OPENSEARCH_URL=$LOCAL_OPENSEARCH_URL" --memory=6g panduk/mq-experimental:0.0.1 &
